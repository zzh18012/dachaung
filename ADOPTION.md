...
## 六十六、Stage 9 立项裁决与批次 26 步骤 1 设计（2026-09-02）

**背景**：项目阶段性收尾后用户下达新指令——对照 2026 年创新实践项目申请
书目标（内容保密，仅技术摘要入裁决循环）做差距分析并补齐结题缺口。差距
分析结论（Claude 实测口径）：解析层 ~90%、结构分块 ~80%、评测体系 ~60%；
语义软分割与 ARI 验证 0%、Web UI 0%（范围原排除）、KVFS 模拟通路 ~20%、
20 篇基线对比实验 ~30%、流式增量 0%（以批处理并行替代）。

**Stage 9 立项裁决（2026-09-02 GPT，有条件 APPROVE）**：
- 目标限定为按申请书验收口径补齐缺口，不回溯修改已封口的 Stage 1–8；
  OCR/扫描件与真正流式增量列为结题限制；未完成实测前不得宣称 ARI ≥75%。
- 批次顺序重排：**26 → 27 → 30A → 28 → 30B → 29 → 31**（30 拆两闸门：
  30A=知识单元最终契约+PDF 字体大小元数据；30B=KVFS 模拟写入接口与链路
  测试；28 须用最终稳定格式）。每批仍走设计汇报→实现→实测→偏差→封口。
- 决策点：A=**A4 架构（可选依赖门控）+ A2（轻量 ONNX）为正式语义后端**，
  模型版本/文件哈希/设备/相似度口径必须锁定，有 embedding 跑正式语义模式
  参与验收、无 embedding 明确标 structural-only 不得宣称语义达标，A3 仅
  降级/对照、A1 仅备选实验；**实际新增依赖仍须用户批准，批次 26 不受阻、
  批次 27 实现前必须完成**。B=Web UI 范围修订批准（FastAPI+静态页，验收
  仅上传/进度/分块预览/引用关系/JSON 导出；需大小限制、临时目录清理、
  一条可重复 smoke test）。C=混合语料来源（Claude 整理公开可再分发文档、
  用户供私有资料；每篇记来源/许可证/抓取日期/SHA-256/格式/版式标签）。
  D=**批次 26 仅授权步骤 1 设计汇报**，未授权冻结 manifest、批量下载、
  实现代码、新增依赖；候选清单可准备。
- 步骤 1 设计汇报七要件：语料构成表（≥20 篇三域+版式分布）、manifest
  与冻结协议、句子级人工标注格式（含投影规则与切分器版本）、holdout 纪
  律（14/6、三域覆盖、人工推导、机械对照另设 split）、零依赖基线定义
  （参数冻结+搜索范围+dev/holdout 规则）、质量与复核方案（异常目录+双标
  注仲裁）、提交物与停机门槛。

**批次 26 步骤 1 执行（本提交）**：docs/stage9-batch26-design.md（完整设
计：24 篇候选清单未采集、manifest 字段与冻结协议、标注 schema 与句子切
分器 v1、预测块→句子投影规则、14/6 分层 split、B1/B2 零依赖基线冻结配
置、异常处理目录与 4 篇双标注复核、校验脚本契约规格（实现时点已向裁决
方提澄清）、风险清单含三项停机门槛）。分支 integration/stage9-batch26-
corpus-annotation（自 main=6c6d398）；未采集任何语料、未冻结 manifest、
未实现代码、未新增依赖。设计汇报提示词经用户中转 GPT 待裁决。

**步骤 1 修订裁决与 v2 落实（2026-09-02，同一分支第二提交）**：GPT 对
步骤 1 设计下达有条件 APPROVE（三项批准条件），v2 修订已全部落实：
- 条件①（数量矛盾澄清与扩容）：首报提示词存在**汇报侧计数笔误**——正
  选 20 被误计为 8+6+5=19（设计文档本身为 8+6+6=20，非设计矛盾），如实
  登记不辩解。按裁决指令改为**冻结 24 篇、三域各 8**：候选表扩为 24 正
  选（学术 8/技术 8/产品 8）+ 备选 4，新增候选 #15（EN 政府公开安全通
  告，CISA 类）、#16（CN 公开统计公报）、#20（PostgreSQL 官方手册）、
  #22（开源项目 DOCX 手册）；DOCX 正选候选 5 篇（#12/#13/#22/#23/#24，
  含用户私有 1），停机线 ≥4。
- 条件②（split 纪律修订）：24 篇 → **dev 14 / comparison 4 / holdout
  6**，三域三 split 全覆盖；用途矩阵=参数搜索仅 dev、机械对照 dev+
  comparison、holdout 仅最终评测一次且报告封存。
- 条件③（事后剔除禁令）：只有冻结前可客观判定的问题允许替换（许可证
  不通过/文件损坏/人眼可判乱码/近重复，逐例登记客观理由）；解析失败/
  空结果/图表零提取/缺页乱序一律**保留并计入解析指标**（选择偏差防护）。
- ARI 原子单位（裁决细化）：标题=text unit 参与 ARI；图像/表格=non-text
  unit 不参与 ARI、仅挂 gold_segment 计关联指标；跨块 unit 最大重叠投影
  规则须配套 fixture 测试（本轮为规格、实现随 ARI/校验脚本子阶段提交）。
- 校验脚本维持契约规格、本轮不实现（裁决确认原判）；下一子阶段授权范
  围=①公开语料采集（逐篇核实许可证）②用户提供私有 DOCX③manifest 定稿
  哈希登记冻结④校验脚本与标注指南实现⑤人工标注与 split 落盘⑥基线配
  置冻结——**待本修订 commit 获裁决方确认后生效**；embedding 依赖仍不
  授权（批次 27 实现前须用户另行批准）。
- 停机门槛四项：许可证合格文档 <20 篇；DOCX <4 篇；双标注 unit 级一致
  率 <85% 且仲裁无法收敛；冻结后发生未经裁决的 manifest 改动。
本修订仅涉及 docs/stage9-batch26-design.md（v2）与本台账补记；仍未采
集、未冻结、未实现、未新增依赖。修订汇报提示词经用户中转 GPT，请求确
认修订 commit 并激活下一子阶段授权。

**步骤 1 封口与下一子阶段授权生效（2026-09-02 GPT 裁决）**：修订
commit 062b654（062b6540150640eb612c3b25352743f460ea1e7c）APPROVE，
批次 26 步骤 1 封口。三项批准条件确认满足（24 篇正选三域各 8+4 备选；
split 14/4/6 且机械对照不进 holdout；解析期表现不事后剔除、保留计指
标；标题纳入 ARI、图像/表格排除、投影 fixture 随实现提交；校验脚本维
持契约规格）。**下一子阶段授权生效**：①采集候选公开语料并逐篇核实许
可证；②接收用户提供的私有 DOCX；③定稿 manifest、计算哈希、登记
ADOPTION.md 并冻结；④实现校验脚本与正式标注指南；⑤完成人工标注并落
盘 split；⑥冻结零依赖基线配置。**仍不授权新增 embedding 依赖**（批
次 27 实现前须用户另行批准）。实施时补两项记录（不重开步骤 1）：
A. manifest 冻结前给出 24 篇逐篇 split/域/格式分配表；B. 明确解析失败
...
## §八十七（2026-09-07）抽查渲染工具补丁：notes 口径块入报告

用户抽查 prod-05 时发现报告页序从 p1 跳 p3（目录页 p2 缺席），
追问"目录你没有记录吗"——排除本身是标注口径（notes 已记"目录页
（p2）与页码未入句子流"），但渲染工具不显示 notes，抽查人无法直接
看到排除口径，属工具透明度缺陷。本 commit 修复（普通工具维护，
零冻结物接触）：

1. `scripts/stage9_annotation_report.py` render() 在头部统计后新增
   notes 块（"—— 标注 notes（处理口径——排除项/判定依据）——"，
   逐行缩进呈现）；无 notes 字段时不输出该块。
2. 测试同步：fixture 增 notes 字段 + 输出断言；全量回归
   5623 passed + 4 skipped（无新增测试数，断言并入既有 4 个）。
3. 两份默认抽查件已重渲染（行数 136→138 / 1157→1159）并同步
   桌面抽查文件夹；annotation_sha256 不变（输入标注字节零改动，
...
## 一百零六、二十六轮确认登记（Batch 3 关闭收讫）

日期：2026-09-11。裁决方式：用户中转 GPT-5.6 Sol（思考程度：高）。
确认内容（照录）：

"收讫。P4' 核验、Batch 3 关闭及台账登记均符合既定裁决。
Stage 10 保持待命；不启动新实质批次，等待 G⑥ 前置到件或新的、
附范围说明的候选申请。"

登记说明：
1. 二十五轮全部执行经确认合规（P4' 四步核验 / Batch 3 关闭 /
   §一百零五台账），无偏差、无补修。
2. Stage 10 待命维持；候选 §1/§2/§3/§6/§8/§9 排队（§6 无
   PyYAML 依赖批准不得开工）。
3. 本节为纯台账登记，无 push 请求；Stage 9 台账队列现为 11 个
...
## 一百一十九、r36 裁决执行：Stage 10 批次 4 跨页表格保守合并实现完成（2026-09-12）

三十六轮裁决（用户中转）：**Stage-10-Batch-4 AUTHORIZED**——分支
integration/stage10-batch4-cross-page-tables 基点 46672e4（累积
Stage 10 链）；real-01 旧 devset 只读判读 + 改后对照（仅限该文档）；
合成夹具测试；合并策略保守（唯一、可解释的连续性证据，证据不足
一律不合并）；首报须披露判定信号/误合并防护/表格计数 delta/全套
回归；source_locator schema/契约变更须停链单独申请。

**执行（实现 commit 9eb4d13，4 文件 +527/−4，未推送）**：

1. **判定信号**（app/parsers/fallback_parser.py）：结构前提 = 页号
   恰 +1（阅读序单链迭代；实证发现 find_tables 页内返回序非阅读序，
   按 (page, top) 稳定排序后消费——实现期真实缺陷修复）+ 列网格
   逐边对齐（单元格 x 边界单遍聚类，边界数相等且逐边 |Δ|≤2pt）+
   候选至少一行非空；连续性证据二者居一：a) 重复表头（候选首行
   空白折叠+casefold 后 == 链首片段首行，且候选 ≥2 行 → 合并并
   丢弃候选首行）；b) 断版位置（链尾底边 ≥80% 页高且候选顶边
   ≤20% 页高）。阈值常量 _XPAGE_GRID_TOL_PT=2.0 / BOTTOM=0.80 /
   TOP=0.20。
2. **误合并防护**：隔页/同页后继表关闭链；网格超容差不合并；中部
   同网格不合并；仅表头无数据行不合并；页内任何表先关链再开新链
   （单链迭代结构性保证）。测试逐项覆盖。
3. **产物表示（零 schema/契约变更，r36 边界内）**：首片段 element
   就地扩展（content 重排、row_count/col_count 重算、metadata 增
   cross_page_merge/continuation_pages/dropped_header_rows）；
   locator 保持首片段起始页；续页 element 移除；合并发生在 relation
   匹配前（题注/引用关系针对合并后表格计算）。
4. **测试**：tests/test_pdf_cross_page_tables.py 13 个（9 个 PDF
   端到端 + 1 个管线 schema 通过 + 3 个判定纯函数单元），全合成
   手写最小多页 PDF 夹具，零真实语料；定向 13/13，全套回归
   **5538 passed + 26 skipped**（基线 5525 + 恰 13 新增，零回归）。
5. **real-01 对照（授权范围，只读）**：表格数 **4→4、合并 0 次**
   （planner 空组）。**诊断更正**：BACKLOG §3 原"+300% 跨页拆分"
   诊断不成立——修订历史表始终完整在 p2 单页，DOCX 1 表 vs PDF
   4 表的 delta 根因是 3 个单柱（2 行×1 列）高亮/提示框被
   find_tables 误检（p6/p17/p17 假阳性）。跨页拆分在该文档无实例；
   合并能力由合成夹具验证。BACKLOG §3 已更新并登记遗留新缺陷
   （单柱/单格假阳性表误检，需单独授权立项）。
6. docs/BACKLOG.md §3 状态更新 + 诊断更正 + 遗留项登记；
   CLAUDE.md 新增批次 4 节。
...
## 一百二十一、r40 裁决执行：假阳性候选方案 0 + Stage-10-Batch-4-Closed（2026-09-12）

四十轮裁决（用户中转）两项：

**1. False-Positive Candidate STATUS（方案 0）**：当前不修，候选
继续挂起。裁定理由：调查已证明纯形状无法区分真 (2,1) 单列表与
假 (2,1) 框；方案 1（min-cols≥2）会误删未来真实 PDF 单列表，
方案 2（文本启发式）违反"宁缺勿猜"。**不得实施任何过滤**。现有
4 个假阳性作为已知限制记录；real-02 PDF 欠检测另立方向不并入
本候选。
执行：stage10 分支 docs commit c908858（未推送）——BACKLOG §3
遗留项改记方案 0 裁定 + 挂起状态 + 4 假阳性已知限制；新增 §3a
real-02 PDF 表格欠检测独立条目（挂起，未立项）。零代码改动。

**2. Stage-10-Batch-4-Closed**：P10' 两笔推送均已完成标准核验，
实现与披露链完整闭合。**关闭记录要件（永久保留）：real-01
表格数 4→4、合并 0 次；诊断更正 = +300% 根因为 3 个单柱假阳性
框非跨页拆分；不得宣称真实文档收益**。审计链
7328566→5d40803→010175d→3f7801d→46672e4→9eb4d13（远端
integration/stage10-batch4-cross-page-tables = 9eb4d13）。
Batch 4 全链：r36 授权 → 实现 9eb4d13（r37 首报四项强制披露）→
r38 追认 + P10' push → r40 关闭。

本节为纯台账 commit；stage10 分支 c908858（纯文档）随下轮一并
申请 push（r14 规则）。状态：G⑥ 双前置（prod-05 delta + G⑤
四篇）在用户侧；C2-C5 待命；C6 需用户批准 PyYAML；Stage 11
Batch 2 未启动；Stage 10 无进行中批次，待命。

## 一百二十二、r41 纯确认：Batch 4 关闭链合规确认（2026-09-12）

四十一轮裁决（用户中转）：收讫。方案 0 落档、§3a 独立挂起及
Batch 4 关闭登记均符合裁定；真实收益声明边界已正确保留。

状态：Stage 10 继续待命；stage10 分支 c908858 与台账（480e7d2/
...
## 一百二十五、r47 纯确认：Batch 5 关闭链合规确认（2026-09-12）

四十七轮裁决（用户中转）：收讫。r45 所有裁决均已按要求执行——
P11' A/B 两笔推送均完成四步核验且无 force；Batch 5 关闭登记有效，
serve"未直接验证、合流时补测"的限制已正确保留；Stage 10 继续
待命，§124 台账提交按既定规则等待下一个实质到件搭车；G⑥ 前置、
C2/C3/C5、§3a、C6 与 Stage 11 Batch 2 状态均不变。

状态：Stage 10 无进行中批次，待命；台账队列 = §124（5a05acb）
+ 本节，随下个实质到件搭车 push。本节为纯台账 commit。

## 一百二十六、r48 纯确认：§125 登记与状态确认（2026-09-12）

四十八轮裁决（用户中转）：收讫。§125 纯台账登记与当前状态均
确认无误。Stage 10 继续待命；§124/§125 按 r14 规则等待下一个
实质到件后搭车推送。
...
## 一百三十、r53 回执批复：协议 v2 与 f 首轮追认合格 + AUTONOMOUS_LOOP.md 一次性例外（2026-09-14）

五十三轮（用户中转，对 r52 附问执行回执的批复）。要点：

- 追认合格：协议 v2 生效与 f 首轮（测试修改、重复审计、
  重名修复、新颖性门槛元测试）均在自跑线授权范围内；后续
  按 f>b>a>c>e 继续；
- **边界事项记明**：AUTONOMOUS_LOOP.md 不属于严格的
  tests/ + STATE.md 产出边界；本次修改追认为**仅用于持久化
  R52 运行协议的一次性协议文件例外**，不开放一般性 docs/
  修改权限；该文件今后非机械性改动须回指示线裁决；
- f 后续（耗时漂移、私有路径审计）仍受"不读 Stage 9 私有
  gold / 冻结面"限制；跨文件重复存量清理留待指示线另行裁决；
- r52 主件 P12' push 授权仍待裁决（convergence 6c58c59 +
  台账 §124–§129 队列不动）。
...
实质 push 搭车。
## §一百三十二（2026-09-16）外部强 AI 评审到件（用户中转，第 33 件）：评测有效性为结题最大风险——两项机械主张本地已核实成立

来源：用户将 2026-09-15 完整简报交外部强 AI 咨询后中转回评审全文。
评审确认工程闭环完整，但判定"结题最容易被质疑的是评测的有效性，
不是代码量"。

**本地已核实（2026-09-16）**：

- **编号不变性缺陷（成立）**：stage9/agreement.py:283 `_consume`
  要求对齐对 gold_segment_id **字符串全等**；合成复现（3 unit 同
  分组，仅 g01/g02→g03/g04 改名）agreement 1.0→0.0、decision
  pass→below_threshold、segment_diff=3。分组语义完全相同的两份
  标注得 0 分。是否影响 acad-03 实测 0.2532 须核对私有标注，
  但 G⑥ 前必须先做编号不变性审计（辅助指标：按分段等价类比较
  而非标签字符串；原报告保留、修订须披露并重算，七项冻结规则
  不静默替换）。
- **main README 滞后（成立）**：main 6c6d398 README.md:5 仍写
  "当前阶段（最小闭环）"，Stage 9–11 已在独立分支未反映。

**登记裁决候选（R-A…R-I）**：R-A 编号不变性审计（G⑥ 前置）；
R-B gold 与 chunk 粒度差诊断（G⑥ 后 dev only：段长分布 +
max_chars≤2000 下边界对齐的诊断上界；不改 ARI 分母宣称达标；
示意：14 等大段精确拆 2 chunk ARI≈0.646、拆 5 ≈0.306）；R-C
以人工语义对象为基准的提取 precision/recall（一对一匹配、
TP/FP/FN、零预测 N/A 口径、按 PDF/DOCX 三域披露——题注关联率
不能代替图表提取率）；R-D G⑥ 凭证机械化（领域覆盖、逐边检查、
每篇负例数量、defects_found 入凭证、SHA 不一致升级）；R-E
B1/B2 输入口径（gold 字符流 vs 原文件解析——须另加"系统解析
文本输入的固定长度对照"并与冻结 B1/B2 分开报告；系统 ARI 与
一致率 0.2532 不可直接互比）；R-F gold 措辞降级为"经人工复核
与仲裁的参考标注"（Claude 草案 + 抽查不等于已证明 4.8 万 unit
普遍一致；分歧/仲裁依据/剩余不确定性原样呈现）；R-G 冻结纪律
（r1 与预注册字节保留、修订走披露过 dev 试验次数的 v2 批次另签
规格、gold 缺陷走 gold-r2 哈希链、holdout 留最终配置首跑）；
R-H 展示（main README 更新、演示页默认 parser auto、
chunk→source spans→locator 回溯视图优先于扩 WebUI）；R-I 自跑
测试分类（测试数不作效果证据；第三方内部行为锁定测试按运行
耗时/升级噪声/是否抓到真实缺陷筛选，直接影响自跑线 a 队列
选向——1b/f 质量审计队列权重上调）。

**评审建议执行序**：编号不变性审计与评测协议 → 四篇真人标注
和仲裁 → 强化抽查凭证 → G⑥ → G⑦ 与粒度诊断 → 有依据的算法
批次 → 最小集成演示。若首轮 ARI<0.75，公开差距、覆盖率与原因
分析，不事后调整指标口径。
...
- **采纳修复（8）**：C06（倾向恢复 tolerance_chars 落盘——CLAUDE.md 已
  形成外部承诺；若确认纯内部指标则转文档）、C08（行空表跳过 + 结构化
  warning）、C09（被剔文件入 summary/event 记录）、C10（过滤侧加
  is_file() 优先）、C12（--log-file 目录/空串错误结构化封装，batch 与
  evaluation 双通道）、C15（坏 annotation 与缺 annotation 区分——评测
  有效性核心面）、C16（annotation doc_id 运行时交叉核对）、P-R2045-1
  （README 参数化；CLAUDE.md 是否修改按其是否属出库文档定）
- **仅改文档（6）**：C01 / C02 / C03 / C04 / C05 / C07
- **缓议（2）**：C11（先定义插件生命周期异常策略）、C13（等 jsonlog
  使用面扩大再处理）
- **仅存档（3）**：C14 / G02 / G05
- **语料治理**：G03 可执行（先行）；G01/G04 改造后执行（G01 先建替代
  测试覆盖映射再拆迁；G04 依 G03 小批验证，不直接总压缩）

问询②待命方向：排序 C > A > D > B。**方向 C（R-I 语料清理执行线）
采纳**——G03 → G04 渐进路线；A 维持为安全模式；D 无必要；**B（重投
D4/P3）否决**（无新事实依据，不为投而投）。

G03 执行协议（自跑线）：

- 待命状态：全队列阻塞待命 → **G03 小批验证循环**（L1 克隆冗余删除）
- 批次：按 clone group 数 / 模块边界切，不按文件数；第一阶段每轮
  500–1000 函数或 ≤1% 语料；连续 5 批稳定（收集锚 / 无缺陷遗漏 /
  代表覆盖）后升至 2000–5000/批；**禁单轮 >5% 语料变化**
- 每批：collect-only 收集数变化记录（+3xN 预测链仅过渡指标）+ 定向
  回归（删除前后代表测试一致性）+ 克隆团代表覆盖复核 + 一批一
  commit + 失败回滚不 squash
- 新基线切换：连续 3 批无收集异常 + 代表覆盖率稳定 + 全量
  collect-only 重建 baseline；此前不替换旧基线
- 约束：不碰 main、不 push main（自跑线照常 push 自身分支）、一批一
  commit；09-21 周期简报保持原计划（R2053 在飞，不受影响）
...
## §一百四十二（2026-09-18）r57：G03 跨文件阈值放宽 ≤3→≤5（保守加强版）+ 09-28 全量实跑一次性豁免

来源：用户中转 r57 裁决（outputs/gpt_brief_autotrack_r57_reply.txt）。

**G03-R57-①**：跨文件完整团阈值批准 ≤3 → ≤5；其余安全条件零
放宽（完整团/规模 2–5/模块层/非 parametrize/恰 1 收集当量/
decorator 守卫/三类安全跳过）。执行约束：批边界以完整团为原子
（不得拆团凑数）；单批硬上限 2000 且单轮 <5% 语料；扇出递进
（先 4-file 团后 5-file 团，3 批亦可不强求 2 批）；每批 collect-only
差值 = 删除函数数精确一致；定向回归并集扩至**所有被改文件**及
相应代表文件；首批出现任何 collection 漂移 / guard 异常 / 未见
回归模式 → 立即停止后续 ≤5 批次（阈值授权 ≠ 必须删完）；≤5 清空
后重算 eligible，若再断供则 G03 在该约束层正式收官，**不自然外推
≤6/≤7**，进一步扇出扩张须重裁。

**G03-R57-②**：2026-09-28 周期基线重建获一次性 ≤60 min 全量
实跑豁免——单次全量 pytest、固定 HEAD、硬超时 60 分钟、禁止
自动重试；成功 → 新 execution baseline；超时 → 如实记 full-run
timeout，简报降级用 collect-only 锚 + 分批定向回归证据并显式注明
"本周期全量 execution baseline 未成功重建"，是否再给窗口另裁。
后台跨轮收割（b）否决（启动轮/证据轮不一一对应、归属复杂、
绕纪律而非处理例外）；常规单轮 ≤30 min 纪律不变。
...
## §一百四十六（2026-09-20）r61：P14' A/B 执行回执 + Stage-10-Batch-7-Closed + Batch 8（§2a 设计/证据轮）授权

来源：用户中转 r61 裁决（outputs/gpt_brief_batch26_r61_reply.txt，
对 r61 首报〔批次 7 首报 + P14' push 申请〕的回复）。批次 7 实施
结果（cae6ef5 纯测试债务）追认。

**① P14' A 执行回执**：远端新分支
integration/stage10-batch7-plugin-timeout-tests 已创建并推送至
**cae6ef5**（非 force）。推送前核验 cae6ef5^ = 252b917 精确、新
对象恰 1；推送后 ls-remote 核对 cae6ef58692a...f9089 精确一致。
网络单窗口一次成功。

**② P14' B 执行回执**：integration/stage9-batch26-corpus-annotation
纯 FF 更新 **47ee181..c03e346**，新增集合恰 {c03e346}（恰 1），
ls-remote 核对 c03e34690d...b8fb 精确一致。

**③ Stage-10-Batch-7-Closed**：两项核验条件精确满足，批次 7 自动
关闭生效；按 r49 纯确认不另开裁决轮。

**④ Batch 8 = §2a 设计/证据轮授权**（PDF 页面家具 heading FP；
暂不授权实现）：分支 integration/stage10-batch8-pdf-page-
furniture-design，基点 cae6ef5（P14' A 核验后成立）。边界：
- 目标 = 回答"15 条页面家具 FP 能否用足够通用且低误伤的信号
  解决"，先证据后规则；
- 必须盘清 15 条类别及可用结构信息（页码/页面位置、bbox/版面
  坐标、跨页重复性、字体/字号、块顺序）在当前 PDF 管线何处存在
  何处丢失；
- 可做只读 shadow analysis，**不得改变解析行为**；
- 至少对比"局部无状态信号"与"文档级位置/重复性信号"两类方案，
  给出误伤反例（合法 Page 01 型标题、日期型标题、近页缘正常标题
  等）；
- 禁止用 real-02 逐字字符串、厂商宣传语、具体页码集合硬编码规则；
- **架构门槛**：可靠方案若要求把跨页状态/版面坐标/文档级重复统计
  引入 _classify_pdf_paragraph 接口或通用 heading 框架 → 停在
  设计结论，不实施接口改造，下一轮单独报裁；即使存在真正局部/
  通用/低误伤方案，也先报证据 + 拟议夹具矩阵，再定 implementation
  batch；
- 授权仅覆盖设计/取证/只读验证；不含生产代码行为变更，不含 push
  授权。C3 与 BACKLOG §3a 留候选池不混做。
...
## 一百六十二、G⑤ tech-08 首件回件收发：部分交回核验 + 补交清单（用户指示处理）

日期：2026-09-22。交回件：blocks_tech-08.py（标注人 Mr.Z，
微信收件，sha256 be0ce6c9c1681b205f2688fb6e11e7e904306c18bb
618127d09c2fcf1f09958f），隔离暂存 outputs/g5_incoming_tech08_
r3rev/（沿 g5_incoming 先例，不直接入 annotations-user/）。

1. **核验结果（assemble rc 1 + 对账诊断 diagnose.py，证据在
   暂存目录）**：
   - 指纹核对一致 2c75ab91…（对 r2/r3 绑定 dump，行号体系正确）；
   - 口径覆盖：r3 代表页 14 页中**已标 8 页**（p003/005/008/
     009/011/014/022/030，p008 为隔页全排除 + 2 nontext 单元，
     合法形态）——**缺 6 页**（p050/071/075/078/082/083，共
     279 行完全未处理）；
   - 越界标注：口径外 21 页有标注（p001/004/006/007/010/012/
     013/015-021/023-029，r3 模板中属 EXCLUDE_BASE 预填区）；
   - 结构错误 2 处：entries 块缺第 5 元素（条目边界判断）——
     p9 目录区（口径内）与 p10 核心数据区（口径外）各一，属
     标注人判断不得代填；
   - EXCLUDE 仍为手平（未保留 EXCLUDE_BASE），后果 = p031-083
     整体未处理 3421/5218 行，装配失败。
2. **补交清单（反馈标注人）**：(a) p9 entries 块补第 5 元素
   （条目起点正则或显式行号）；(b) 补标 6 缺页 p050/071/075/
   078/082/083（或按负责人改判口径处置）；(c) EXCLUDE 恢复
   EXCLUDE_BASE | 座位号 结构（或按改判口径整体排除未标页）；
   (d) 疑点答复（按指南原文）：表格不丢——tab:N nontext 单元
   保留表格的阅读位置与计数，仅单元格文字不进句级正文流
   （G⑤ 文本流一致率与 nontext 对齐分开测量）。
3. **越界 21 页处置**：待负责人裁定——(a) 严格口径作废 /
   (b) 改判 p1-30 为本篇页集 / (c) 并集保留全部工作 + 补 6 页。
   改判/并集均须同步修订 G⑤ 同页集对齐口径（§149⑤）并随
   简报披露。
