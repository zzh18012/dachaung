# -*- coding: utf-8 -*-
"""Stage 6 批次 7 holdout（holdout-table-caption-relation）一次性首跑。

纪律（docs/table-caption-relation-contract.md §6 + 2026-08-30 裁决⑤）：
- 期望 relations/elements 在本夹具任何 parser 运行之前手工推导冻结
  （samples/synthetic/holdout-table-caption/expectations.json），本脚本只比对；
- 合成夹具字节固定：运行时校验 sha256 与冻结登记一致，漂移即拒跑，
  且永不调用生成脚本重新生成；
- 必须在干净工作树、固定干净 SHA 下运行一次，报告封存 outputs/，
  之后永不重跑（输出文件已存在即拒绝）。

比对口径：schema_version（期望 0.5.0）、逐 element 的
element_id/type/source_locator 全等、全部 caption 的 content、
relations 数组全等（含 metadata.rule 与 (type,from,to) 排序）。
"""
...
HOLDOUT_DIR = ROOT / "samples" / "synthetic" / "holdout-table-caption"
OUT = ROOT / "outputs" / "holdout-table-caption-v1-firstrun.json"
