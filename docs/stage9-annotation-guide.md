...
> **术语边界（外部评审 R-F，2026-09-16）**：本文所有 "gold 标注" 指
> **经人工复核与仲裁的参考标注**（annotated reference），不是客观
> 真值（ground truth）——标注质量由双标注一致率、仲裁收敛与抽查
> 凭证（G⑤/G⑥）约束并披露，分歧不可消为零。对外表述（结题报告/
> README/演示）一律用"参考标注"措辞，不得写"真值/标准答案"。
