...
- 根因方向：页脚/封面文字缺少位置（页底边距带/页眉页脚区）与模板
  重复性（同文式跨页复现）信号
- 状态：**设计/证据轮完成（Stage 10 批次 8，2026-09-20，r61 授权，
  零实现）**——只读 shadow analysis 结论：A 页码×12 + C 宣传语×2
  可用"bot ≥93% 底带 ×（页码模板 ∪ 跨页逐字重复）"文档级合取信号
  零合法误伤解决（实测家具底带 95.2–97.6% vs 合法 ≤83.2%）；B 封面
  日期×1 三类结构信号与合法期间标题同形，无低误伤通用信号，建议划出
  范围列已知限制；推荐架构 = _parse_pdf 文档级后置过滤（分类器/通用
  框架零改动，不触发 r61 接口改造停链条款）。设计文档
  docs/stage10-batch8-page-furniture-design.md；实现授权与阈值/
  夹具矩阵待 r62 裁决
