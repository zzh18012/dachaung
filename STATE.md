...
### 目标
- 给 `evaluation/annotation_metrics.py`（194 行）加第二十八轮 edges 测试，覆盖 edges26 未触及的角度：**chunk_boundary_prf 算法精确**（贪心匹配 nearest first / tie break by used_pred / position before/after / position 缺失默认 after / repeated marker search_from 推进 / 重复 marker 在 tolerance 内 / anchor 不共享 stream 位置）；**chunk_boundary_prf f1 各分支**（perfect match=1.0 / zero denominator=0（不是 null）/ p None → null precision_or_recall_not_evaluated / partial match / tight tolerance）；**chunk_boundary_prf 输出 dict 结构精确**（normal keys 4 项 / with missing markers / no missing when all found / each metric 是 dict 含 value+reason）；**chunk_boundary_prf 边界 case**（chunks empty text / document 无 chunks key / annotation 无 anchors key / _tolerance_chars 在所有 6 路径都写）；**figure_caption_prf source level**（signature / 用 dict literal 构造 / no try except / no loop / 含 3 keys / docstring）；**module source forbidden tokens 第二批**（35 个 stdlib 模块）；**module source 字符串精确补强**（imports / constant / docstring mentions caption/relation/marker/tolerance/heuristic/greedy/一对一 / 各步骤 substring / no yield/global/async/class/decorators/main）；**signatures 精确补强**（3 params + annotations + default 30 + POSITIONAL_OR_KEYWORD + no varargs/varkw + namespace）；**模块整体合理性**（__all__ 3 / 2 functions / 1 constant / no class / no main）；**端到端集成补强**（perfect match 3 chunks / no match tolerance=0 / deterministic / unicode chunks / punctuation / extra whitespace / realistic docx / empty marker missing / partial match 5 chunks / realistic figure_caption input）

### 改动
- 新增 `tests/test_annotation_metrics_edges27.py`（119 测试）
- 仅测试，不动业务代码

### 覆盖要点
- **chunk_boundary_prf 算法精确（贪心匹配）**：7 测试
- **chunk_boundary_prf f1 各分支**：5 测试
- **chunk_boundary_prf 输出 dict 结构精确**：4 测试
- **chunk_boundary_prf 边界 case**：4 测试
- **figure_caption_prf source level**：6 测试
- **module source forbidden tokens 第二批**：35 测试（parametrize）
...
### 测试基线
- main：163 pass / 0 fail / 0 skip（HEAD `2c35244`）
- 本 worktree（Round 327 后）：32336 pass / 0 fail / 18 skip（HEAD `ed2c240`）
...
**建议**：annotation_metrics edges27 已饱和（贪心匹配各 case 7 + f1 分支 5 + dict 结构 4 + 边界 4 + figure_caption source 6 + forbidden 35 + source 字符串 30 + signatures 11 + 模块 6 + 端到端 11）。下一轮选 evaluation/metrics.py 第二十八轮。
...
### 目标
- 给 `evaluation/schema.py`（80 行）加第十六轮 edges 测试，覆盖 edges15 未触及的角度：**EvalSchemaError 行为深度补强**（isinstance Exception；errors 默认 [] is list；errors=[{}] 保留伪 truthy；errors 可访问 + 可修改；__str__/__repr__ 含 message/class name；args 含 message；MRO 直接 base Exception；init signature 2 params；no varargs/varkw；unicode/emoji/long message）；**EvalSchemaError 深度边界补强**（[] vs None 等价；[{}] 单个；tuple/dict/set/str truthy 保留原值；空字符串/1000 字符/emoji message）；**_schema_path 行为深度补强**（返 absolute Path；is_file True；name 正确；signature 1 param；source 含 SCHEMAS_DIR + .is_file() + FileNotFoundError + 中文「Schema 文件不存在」）；**load_schema 行为深度补强**（4 schema 都加载；返 dict 不缓存；含 $schema/$id/type；signature 1 param；source 含 utf-8 + json.load + _schema_path 调用）；**validate 行为深度补强**（不修改 instance；可重复调用；errors path/message/schema_path 都是 list/str/list；message 含 schema_name + 数量；signature 2 params；source 含 Draft202012Validator + iter_errors + sorted + absolute_path + head=errors[0]）；**validate 错误聚合深度补强**（1 个错误 / 多个错误按路径排序 / flat error keys 精确 / head.message 是 str）；**validate_file 行为深度补强**（Path/str 都接受；不存在 → FileNotFoundError；目录 → FileNotFoundError；invalid JSON → JSONDecodeError；signature 2 params；source 含 Path(path) + .is_file() + utf-8 + json.load + validate 调用）；**Draft202012Validator + JSValidationError 行为深度补强**（都是 class；JSValidationError issubclass Exception；无继承关系；source 含 from import + 不含 import as）；**module __all__ 完整性补强**（5 entries 顺序；namespace；SCHEMAS_DIR 是 Path；EvalSchemaError 是 class；3 callable；_schema_path 私有）；**SCHEMAS_DIR 行为深度补强**（absolute；resolve 后绝对；parent 是 project_root（含 pyproject.toml）；name='schemas'；含 4 个 .json；无 .py；source 含 Path(__file__).resolve() + .parent.parent）；**schema 文件内容深度补强**（manifest_version const "1.0" + documents array；evaluation-report 含 provenance + per_doc array；annotation 含 doc_id + chunk_boundary_anchors；document 含 source_type + elements/chunks array）；**module source forbidden tokens 补强**（os/sys/socket/email/html/http/urllib/sqlite3/csv/pickle 10 个）；**module imports 顺序 + future 补强**（6 imports 精确：future/json/pathlib/typing/jsonschema 2 行；不含 import as）；**module docstring 深度补强**（含 manifest/annotation/evaluation-report/app/schema.py/业务/评测）；**signatures 精确补强**（5 callable 各 param 名 + no varargs/varkw；validate_file path 注解是 Path | str）；**module source level 完整补强**（EvalSchemaError 含 super().__init__ + self.errors = errors or []；_schema_path 含 SCHEMAS_DIR + .is_file() + raise；load_schema 含 with .open utf-8 + json.load；validate 含 5 步算法；validate_file 含 Path + .is_file() + .open + validate 调用）；**端到端集成补强**（3 schema happy path；cross-schema 失败；errors count 随 violation 增加；validate_file with Path object）；**模块整体合理性**（1 class + 3 public function + 1 private helper + 无 main + 唯一 constant SCHEMAS_DIR）

### 改动
- 新增 `tests/test_evaluation_schema_edges16.py`（137 测试）
- 仅测试，不动业务代码

### 覆盖要点
- **EvalSchemaError 行为深度补强**：12 测试
- **EvalSchemaError 深度边界补强**：9 测试
- **_schema_path 行为深度补强**：7 测试
- **load_schema 行为深度补强**：8 测试
- **validate 行为深度补强**：12 测试
- **validate 错误聚合深度补强**：4 测试
- **validate_file 行为深度补强**：10 测试
- **Draft202012Validator + JSValidationError 行为深度补强**：7 测试
- **module __all__ 完整性补强**：6 测试
- **SCHEMAS_DIR 行为深度补强**：7 测试
- **schema 文件内容深度补强**：9 测试
- **module source forbidden tokens 补强**：10 测试
- **module imports 顺序 + future 补强**：6 测试
- **module docstring 深度补强**：6 测试
- **signatures 精确补强**：7 测试
- **module source level 完整补强**：5 测试
- **端到端集成补强**：5 测试
- **模块整体合理性**：5 测试

### 撞墙记录
- 10 fail 首次跑：
  - manifest schema 必须有 `devset_status` field（5 处测试需要加）
  - `SCHEMAS_DIR.parent.parent` 不是 project root（`SCHEMAS_DIR.parent` 才是）
  - annotation schema 用 `doc_id` 不是 `document_id`
  - 6 imports 不是 5（jsonschema 用 2 行：Draft + ValidationError）
- 修复后 137 全通过

### 测试基线
- main：163 pass / 0 fail / 0 skip（HEAD `2c35244`）
- 本 worktree（Round 305 后）：29496 pass / 0 fail / 16 skip（HEAD `409286a`）
...
**建议**：schema.py edges16 已饱和（EvalSchemaError 行为/边界 / _schema_path / load_schema / validate 错误聚合 / validate_file / Draft+JSValidation / __all__ / SCHEMAS_DIR / schema 文件内容 / forbidden tokens / imports / signatures / source level / 端到端 / 模块整体）。下一轮选 evaluation/runner.py 第二十五轮，覆盖 _load_annotation + _process_one 5-tuple 返回 + run_evaluation keyword-only args 各分支深度。
...
### 目标
- 给 `evaluation/metrics.py`（381 行）加第二十三轮 edges 测试，覆盖 edges21 未触及的角度：**4 helper one-liner 行为深度补强**（_null 返独立 dict；_ratio 接受 int/负数不 clamp；_bool_metric 接受 int 0/1 + truthy dict 返 bool 类型；_int_metric 接受 float 截断 + 负数 + str 抛 ValueError + 返 int 类型）；**3 常量精确补强**（_TEXT_TYPES 7 entries 顺序精确 + 不含 image；_PDF_BBOX_REQUIRED_TYPES 4 entries 顺序 + ⊂ _TEXT_TYPES；_NOT_EVALUATED='not_evaluated'；tuples 类型；str 类型）；**compute_automatic_metrics 边界组合补强**（document None + error dict 含 code → pipeline_success=False + error_code=error['code']；document dict + error dict → pipeline_success=False；source_type='unknown' → pdf + docx locator 都 null；source_type='pdf' → docx null；source_type='docx' → pdf null；空 document dict → no_elements/no_chunks；error_code None when no error；返 dict 至少 13 keys）；**schema_valid 行为深度**（document None → null pipeline_failed；document 通过/失败 → bool；value 类型 bool）；**_pdf_locator_ratio 行为深度补强**（elements=[] → null；全 valid/invalid/mixed；page=None/0/-1/'1'(str) → invalid；非文本类型不需 bbox；table 类型 page valid）；**_docx_locator_ratio 行为深度补强**（elements=[] → null；全 valid structural_keys；含 page/bbox → invalid；无 structural_keys → 0.0）；**_is_valid_bbox 行为深度补强**（None/not list/wrong length/bool/str/NaN/Inf/int/float 各分支）；**_image_resource_ratio 行为深度补强**（无 image → null；image 无 resource_path → 0.0；文件存在/size=0/不存在；image_base_dir 拼接；混合 valid+invalid）；**_chunk_reference_ratio 边界补强**（chunks=[] → null；elements=[] chunks 有 ids → 0.0；all valid/partial unknown/empty source ids/None source ids/重复 id）；**_heading_boundary_ratio 边界补强**（无 headings → null；无 chunks → 0.0；perfect/partial match；chunk first id 空字符串不匹配；chunk id 不在 first 位置；set 去重）；**_silent_drop_count 边界补强**（expectations None/{}/no element_count_key/element_count None/{}；by_type={} 全 drop；actual==expected → 0；actual>expected → 0；多类型 partial 求和；返 int）；**_strip_unicode_whitespace 行为深度补强**（ASCII space/tab/newline/NBSP/em space/ideographic space/line separator/paragraph separator；全空白→''；无空白原样；保留 emoji/中文/标点/数字；空字符串）；**_text_preservation 边界补强**（image 不参与；chunks 全空白 → empty_actual；elements 全 image → empty_expected；都空 → empty_expected_and_actual；Counter 取 min；乱序 → equal False 但 P/R=1.0；返 3 keys）；**module imports 顺序**（future → math → collections → pathlib → typing；5 imports 精确；无 os/sys）；**module __all__ 精确**（1 entry compute_automatic_metrics；namespace/callable/identifier）；**module namespace**（4 helper + 9 metric helper + 3 constant + 1 public function）；**module source forbidden tokens 补强**（re/logging/subprocess/asyncio/threading/json/time/datetime/itertools/functools/relative/class/dataclass/yield/async/global/walrus/assert）；**module docstring 深度补强**（含「自动指标」/「纯函数」/「不修改 document」/「text_preservation」/「Counter」/「Unicode 空白」/「v1.1」）；**signatures 精确**（compute_automatic_metrics 5 params + image_base_dir default=None + no varargs/varkw；4 helper 1 param each；9 metric helper 各自签名精确；13 helper 都 no varargs/varkw）；**module source level 完整**（compute_automatic_metrics lazy import + try/except + 11 metric 名 + pipeline_success 计算；_pdf_locator_ratio _PDF_BBOX_REQUIRED_TYPES/_is_valid_bbox 调用；_docx_locator_ratio structural_keys 7 个；_is_valid_bbox math.isfinite；_image_resource_ratio candidates + Path(rp) + stat().st_size + except OSError；_chunk_reference_ratio set comprehension；_strip_unicode_whitespace isspace；_text_preservation Counter + 交集；_heading_boundary_ratio chunk_first_ids + ids[0]；_silent_drop_count max(0, exp-actual) 计算）；**端到端集成**（PDF document 全 metric 都有值；DOCX locator 1.0；image + tmp 文件 → image_resource 1.0；不修改 document/error；含 expectations → silent_drop 计算；缺 code 抛 KeyError）；**模块整体合理性**（__all__ 1 entry；1 public + 13 private + 3 constant；无 class；无 __main__）

### 改动
- 新增 `tests/test_evaluation_metrics_edges22.py`（194 测试）
- 仅测试，不动业务代码

### 覆盖要点
- **4 helper one-liner 行为深度补强**：13 测试
- **3 常量精确补强**：7 测试
- **compute_automatic_metrics 边界组合补强**：8 测试
- **schema_valid 行为深度**：3 测试
- **_pdf_locator_ratio 行为深度补强**：8 测试
- **_docx_locator_ratio 行为深度补强**：5 测试
- **_is_valid_bbox 行为深度补强**：9 测试
- **_image_resource_ratio 行为深度补强**：7 测试
- **_chunk_reference_ratio 边界补强**：7 测试
- **_heading_boundary_ratio 边界补强**：7 测试
- **_silent_drop_count 边界补强**：10 测试
- **_strip_unicode_whitespace 行为深度补强**：14 测试
- **_text_preservation 边界补强**：8 测试
- **module imports 顺序**：7 测试
- **module __all__ 精确**：4 测试
...
### 撞墙记录
- 1 fail 首次跑：
  1. `test_chunk_reference_ratio_empty_source_ids_invalid` —— 我用 `_make_chunk("a", [])` 构造 chunk，但 `_make_chunk` 内 `cids or ["e1"]` 把空 list 替换成 `["e1"]`，导致 source_element_ids 不是 []。修法：直接构造 dict 而不用 helper。

### 测试基线
- main：163 pass / 0 fail / 0 skip（HEAD `2c35244`）
- 本 worktree（Round 298 后）：28480 pass / 0 fail / 16 skip（HEAD `ea6b359`）
...
**建议**：metrics.py edges22 已饱和（4 helper + 3 常量 + 11 metric 各分支 + signatures + source level + 端到端）。下一轮选 evaluation/schema.py 第十五轮，覆盖 4 个 schema 文件的 cross-validation 与 EvalSchemaError 行为深度补强。
...
- 候选：
  ...
  - evaluation/cli.py 第二十二轮
  - 仍阻塞：J（向量化）、M（evaluator v1.2）、O（docs/*.md）

**建议**：cli.py edges21 已饱和（exit code 矩阵 + _format_metric value 类型完整 + _run_inspect_doc _sort_key 4 分支 + argparse 元数据 + stdout 模板 + module/source level 完整 + 集成 round-trip）。下一轮选 evaluation/annotation_metrics.py 第二十一轮，覆盖 figure_caption_prf + chunk_boundary_prf 行为深度。

---

## Round 290 — evaluation/cli.py 第二十二轮（178 测试）

### 目标
- 给 `evaluation/cli.py`（243 行）加第二十二轮 edges 测试，覆盖 edges20 未触及的角度：**main 深度 - run 路径**（manifest 路径是目录 → exit 2；output 已存在覆盖；evaluator_version 不变；report_version 不变；stderr 含「清单加载失败」「清单不存在」；kreuzberg parser 走通；max-chars 0/negative → schema 拒绝 exit 1；tolerance-chars 0 走通；stdout 含完整模板 documents=/成功/失败/devset_status=/file_count=/groups=/pdf=/docx=/git_commit=/git_dirty=；空 manifest → documents=0 成功 0 失败 0）；**main 深度 - validate-report 路径**（目录路径 → exit 2；stderr 含「报告不存在」「报告校验失败」「JSON 解析失败」；成功 stdout 含「通过 evaluation-report Schema 校验」）；**main 深度 - inspect-doc 路径**（目录路径 → exit 2；stderr 含「文档不存在」「JSON 顶层不是对象」；top-level array/string/int/null/float/bool 全部 exit 1）；**main argv 行为深度**（argv=None 空 sys.argv 抛 SystemExit；argv=[] 抛 SystemExit；未知 command 抛；缺 --manifest 抛；validate-report 缺 input 抛；inspect-doc 缺 input 抛）；**_format_metric value 类型完整**（None/True/False/int 0/int 负/float 0.0/float 负/float 1/3 精度/empty dict/non-empty dict sorted/string fallback/list fallback/long float/tuple fallback 16 场景）；**_format_metric 字符串模板精确**（width 36 严格；长 name 不截断；2 leading spaces；返回 str）；**_run_inspect_doc _sort_key 4 分支**（bool→0/int-float→1/dict→2/null→3）；**_run_inspect_doc 输出 ?**（document_id/source_path/parser_name/parser_version 缺失输出 ?）；**_run_inspect_doc source_type 缺失 default unknown**（pdf/docx/html/missing 4 场景）；**_run_inspect_doc 是 module-level 函数**（不是嵌套；1 param；no varargs/varkw；return int）；**_build_parser 元数据深度**（prog=evaluation.cli；description 不空含「评测」；formatter_class=RawDescriptionHelpFormatter；subparsers dest=command required=True；run/validate-report/inspect-doc 三个 sub-parser 存在；run 有 5 个 option；inspect-doc 有 tolerance-chars 无 parser/max-chars；validate-report 无 optional）；**main 不可达 return 2 + main block**（source 含 return 2；3 处 if args.command；return 0/1；argv 默认 None；return int；no varargs/varkw）；**module source level 完整**（import 顺序 argparse→json→sys→pathlib→evaluation；含 hasattr check；含 stdout/stderr reconfigure；含 AttributeError/OSError catch；含 from evaluation 4 个 imports；不含 logging/subprocess/os/re/time/threading/asyncio/concurrent；lazy imports 在 _run_inspect_doc；main block raise SystemExit；__main__ 在末尾；无 __all__；module docstring 含三个子命令名）；**_format_metric signature 与 source level**（2 params；return str；含 metric.get value/reason；含 isinstance bool/float/dict；含 :.4f；含 sorted）；**_run_inspect_doc source level 完整**（input_path.open/json.load/json.JSONDecodeError/isinstance dict/source_type/compute_automatic_metrics/figure_caption_prf/chunk_boundary_prf/_sort_key/file:/document_id:/source:/parser:/counts:/metrics:/return 0）；**stdout/stderr 模板精确**（[OK] 在 stdout；错误消息只在 stderr；成功时 stderr 无 [ERROR]）；**run 集成 round-trip**（run 后可被 validate-report 通过；空 documents 仍合法；devset_status 传播；output 在已存在子目录正常创建）；**inspect-doc 完整集成**（输出含全 6 个 label；counts 显示 elements=N chunks=N；tolerance-chars 0/-1 都 exit 0）；**run_evaluation 内部输出**（stdout git_commit 12 字符 / git_dirty=）

### 改动
- 新增 `tests/test_evaluation_cli_edges21.py`（178 测试）
- 仅测试，不动业务代码

### 覆盖要点
- **main 深度 - run**：12 场景（目录/覆盖/版本不变/stderr 文案/kreuzberg/max-chars/tolerance/stdout 模板/空 manifest）
- **main 深度 - validate-report**：5 场景（目录/stderr 3 种文案/成功模板）
- **main 深度 - inspect-doc**：7 场景（目录/stderr 5 种顶层类型/无 type dict 走通）
- **main argv 行为**：6 场景（None/empty/unknown/missing required）
- **_format_metric value 类型**：16 场景（None/bool/int/float/dict/string/list/tuple 全覆盖）
- **_format_metric 模板**：4 测试（width 36/long name/leading spaces/return str）
- **_run_inspect_doc _sort_key**：4 测试（4 分支）
- **_run_inspect_doc 输出 ?**：7 测试（document_id/source_path/parser_name/version missing + present）
- **_run_inspect_doc source_type**：4 测试（pdf/docx/html/missing default）
- **_run_inspect_doc 函数属性**：4 测试（module-level/signature/no varargs/return int）
- **_build_parser 元数据**：20 测试（prog/description/formatter_class/dest/required/3 sub-parser/各 sub 的 option 矩阵）
- **main source level**：8 测试（return 2/3 个 if/return 0/1/signature/no varargs/varkw）
- **module source level**：26 测试（import 顺序/hasattr/reconfigure/catch/4 个 from evaluation/禁止 imports 9 个/lazy imports/main block/__main__ 末尾/__all__/docstring）
- **_format_metric signature/source**：15 测试（2 params/return str/get value/reason/isinstance 3 种/:.4f/sorted）
- **_run_inspect_doc source level**：16 测试（open/json.load/JSONDecodeError/isinstance dict/source_type/3 个 metric 调用/_sort_key/6 个 label/return 0）
- **stdout/stderr 模板**：6 测试（[OK]/错误只在 stderr/成功 stderr 干净）
- **run 集成 round-trip**：5 测试（可被 validate-report 通过/空 documents/devset 传播/sub dir）
- **inspect-doc 完整集成**：6 测试（全 6 label/elements=N/chunks=N/最小/tolerance 0/-1）
- **run_evaluation 内部输出**：2 测试（git_commit 12 字符/git_dirty=）

### 撞墙记录
- 2 fail 首次跑：
  1. `test_main_run_with_max_chars_zero` —— argparse 接受 max-chars=0，但 evaluation-report.schema.json 要求 max_chars >= 1。CLI run 在生成报告后自校验失败 → exit 1（不是 0）。修法：assert rc == 1。
  2. `test_main_run_negative_max_chars_accepted` —— 同上，max-chars=-1 → schema 拒绝 → exit 1。修法：assert rc == 1，并改测试名为 `test_main_run_negative_max_chars_rejected_by_schema`。

### 测试基线
- main：163 pass / 0 fail / 0 skip（HEAD `2c35244`）
- 本 worktree（Round 290 后）：27103 pass / 0 fail / 16 skip（HEAD `3aac5cf`）
