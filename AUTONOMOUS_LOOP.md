...
## 协议 v3.3 — R59 周期基线重建裁决（2026-09-24 起生效，增量于 v3.2；未述部分以 v3.2/v3.1/v3 为准）

GPT 裁决（R59，用户 2026-09-24 中转，指示线台账 §179；简报原文 `dachuang-adoption/outputs/gpt_brief_autotrack_r59_cycle_early.txt`）。六项裁定：

**【R59-①】提前执行追认 + R57-② 已消耗**：09-28 窗口提前至 09-23 追认有效；R57-② 一次性豁免绑定"周期基线重建事件"，就此消耗，不得在 09-28 再次使用；09-28 无须补跑、无须空转记录，标记为已由 09-23 提前履行。后续恢复常规无人值守 ≤30 分钟纪律。

**【R59-②】execution baseline 92365 @ c9268f9 批准——强制双层表述，限定词不得省略**：
- 原始执行事实：实际代跑全量 **92311 passed / 2 failed / 52 skipped = 92365，1363.59s**（dachuang-code detached @ c9268f9）
- 两项定向归因：失败两测试在母环境分别 **PASS / SKIP**（环境差异直接复现，非推断）
- 母环境等价 baseline：**92312/53/0 = 92365**——是**归因后的 baseline，不是一次实际完成的母环境全量运行，此区别以后不得省略**
- 2 失败归档为 execution-environment ownership：不进 corpus defect、不触发自动修复批

**【R59-③】G03 与锚记账确认**：G03 维持正式收官（eligible(≤5)=0，不外推 ≤6/≤7；190 parametrize deferred 与 group_size_gt5 不变）；锚记账 = **collect 92365（六连 R2065/66/69/70/71/73）+ execution 92365（带②限定词）**；旧 102091 转 historical。R2073 归属澄清接受：SyntaxWarning 六文件为自跑分支语料侧观察项，不得再描述为 main 侧缺陷。

**【R59-④】下一常规周期窗口 2026-10-05**：此前若发生改变审计前提的事件（main 基准前进 / 新明确裁决到件）可按既有变化触发机制提前进入对应轻量轮；不恢复已消耗的 R57-② 全量豁免。

**【R59-⑤】候选池冻结登记**：本轮全部维持登记、不指定实施窗口（R2042/R2068 22 项、scripts 归档候选、fixture 三项继续 inventory；SyntaxWarning 六文件按 R2073 修正归属继续登记）；**G⑤ 回收不自动构成实施授权**；不因 Stage 11 关闭把自跑线候选反向注入指示线。

**【R59-⑥】零新增授权**：本轮不授权新的测试删除、fixture 清理、scripts 归档、warning 修复或 main 修改；自跑线恢复既定 f/b 常规轮换或轻量健康审计；G04/G05 不动；main 参考 6c6d398 不变时不触发 R2047 重探。

## 协议 v3.2 — R57 G03 扇出放宽执行参数（2026-09-18 起生效，增量于 v3.1；未述部分以 v3.1/v3 为准）

GPT 裁决（R57，用户 2026-09-18 中转，指示线台账 §142；原文 `dachuang-adoption/outputs/gpt_brief_autotrack_r57_reply.txt`）。两项裁定：

**【G03-R57-①】跨文件完整团阈值批准 ≤3 → ≤5，其余安全条件零放宽**（完整团原子、团规模 2–5、模块层、非 parametrize、恰 1 收集当量、R2056 decorator 守卫、既有重名/未收集/被引用三类安全跳过全沿用）。执行约束：
- 批边界以**完整团**为原子，不得拆团凑数；单批硬上限 2000 且单轮 <5% 语料
- 扇出递进：**先处理 4-file 团，再处理 5-file 团**（数量导致 3 批也接受，不强求 2 批）
- 每批 collect-only 差值与实际删除函数数**精确一致**；定向回归并集扩展至**所有被改文件及相应代表文件所在文件**全部跑绿
- **首批出现任何 collection 漂移、guard 异常或此前未见的回归模式 → 立即停止后续 ≤5 批次**，如实记录，不把"已获阈值授权"理解为必须删完
- ≤5 清空后重算 eligible；若再次断供则 **G03 在该约束层正式收官**，不自然外推 ≤6/≤7（任何进一步扇出扩张须重新裁定）

**【G03-R57-②】2026-09-28 周期基线重建获一次性 ≤60 分钟全量实跑豁免**：固定 HEAD、单次运行、禁止自动重试；成功则建立新 execution baseline，超时则如实降级（记录 full-run timeout，周期简报用已验证的 collect-only 95929 + 分批定向回归证据并注明"本周期全量 execution baseline 未成功重建"，不伪造完整基线，同周期不自动再跑）。该豁免**仅此一次**，不改变后续无人值守轮常规 ≤30 分钟纪律；后台跨轮收割方式否决。

背景（R2063 收官评估供参照）：1050 个 guard-eligible 完整团（4–5 文件）/ 3564 函数恰为剩余 eligible 全部；现行 ≤3 约束组合下供给 = 0（R2062 pool 遍尽实测）；一期累计 2500 + 二期 2000 + 1671 = 已删 6171 函数，现行收集锚 95929。190 条 parametrize 延期、source-lock（G01）、B 桶（G05）维持不动。
